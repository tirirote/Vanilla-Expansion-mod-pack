...
# Params: $(model), $(face_player), $(scale), $(random_scale), $(random_pos), $(lifetime)
...
$execute if score #swoosh_face vexp.math matches 1.. facing entity @p[tag=vexp.attacker,limit=1] eyes run summon item_display ~ ~ ~ {Tags:["vexp.swoosh","vexp.swoosh.new"],item:{id:"minecraft:firework_rocket",count:1,components:{"minecraft:custom_model_data":{strings:["$(model)"]}}},transformation:{left_rotation:[0.7071f,0f,0f,0.7071f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},brightness:{block:15,sky:15},teleport_duration:2,interpolation_duration:2,view_range:1.0f}

#Face player = 0
$execute if score #swoosh_face vexp.math matches ..0 run summon item_display ~ ~ ~ {Tags:["vexp.swoosh","vexp.swoosh.new"],item:{id:"minecraft:firework_rocket",count:1,components:{"minecraft:custom_model_data":{strings:["$(model)"]}}},transformation:{left_rotation:[0f,0f,0f,0.7071f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},brightness:{block:15,sky:15},teleport_duration:2,interpolation_duration:2,view_range:1.0f}
...
$execute as @e[type=item_display,tag=vexp.swoosh.new,sort=nearest,limit=1,distance=..1] run data modify entity @s data.swoosh.random_scale set value $(random_scale)
