...
# 2. Debug: Decir mi ID
execute if score @s vexp.id matches 1.. run tellraw @a [{"text":"Interaction ID: "},{"score":{"name":"@s","objective":"vexp.id"}}]
execute unless score @s vexp.id matches 1.. run tellraw @a {"text":"Error: Interaction tiene ID nulo o 0","color":"red"}
...
execute at @s as @e[type=item_display,tag=vexp.chair,distance=..4] if score @s vexp.id = @e[tag=vexp.current_killer,limit=1] vexp.id run tellraw @a {"text":"Modelo visual eliminado","color":"green"}
