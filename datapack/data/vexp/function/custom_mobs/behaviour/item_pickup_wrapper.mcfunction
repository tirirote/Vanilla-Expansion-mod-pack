...
# 2. Move towards item
execute unless block ~ ~-1 ~ #air if block ^ ^ ^0.5 #replaceable run tp @s ^ ^ ^0.3 facing entity @e[type=item,tag=vexp.mob_favorite_item_found,sort=nearest,limit=1]

execute if block ~ ~-1 ~ #air if block ^ ^ ^0.5 #replaceable run tp @s ^ ^-0.5 ^0.3 facing entity @e[type=item,tag=vexp.mob_favorite_item_found,sort=nearest,limit=1]
