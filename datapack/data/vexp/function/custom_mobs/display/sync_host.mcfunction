...
execute as @e[type=item_display,tag=vexp.custom_mob_display,distance=..64] if score @s vexp.id = @e[tag=vexp.custom_mob,nbt=!{HurtTime:0s},sort=nearest,distance=..0.1,limit=1] vexp.id unless entity @s[tag=vexp.mob_display_hurted] if items entity @s contents stone if data entity @s item.components."minecraft:custom_model_data".strings[0] unless data entity @s data.vexp.mob_display.base_model run data modify entity @s data.vexp.mob_display.base_model set from entity @s item.components."minecraft:custom_model_data".strings[0]
...
execute as @e[type=item_display,tag=vexp.custom_mob_display,tag=vexp.custom_mob_inventory,distance=..64] if score @s vexp.id = @e[tag=vexp.custom_mob,tag=!vexp.mob_got_item,sort=nearest,distance=..0.1,limit=1] vexp.id run tp @s ~ ~300 ~
