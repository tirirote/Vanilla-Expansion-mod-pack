...
execute positioned ~ ~-1 ~ if entity @e[type=!player,type=!item_display,type=!interaction,type=!marker,distance=..0.75,sort=nearest,limit=1] run function vexp:dungeons/dagger/projectile/hit_mob

# Comprobar si hay bloques
execute positioned ~ ~.5 ~ unless block ^ ^ ^1 #minecraft:replaceable run function vexp:dungeons/dagger/projectile/hit_block
