...
# Mirror owner metadata so probe-based hit hooks can resolve vexp.arrow.owner.
execute if data entity @s data.vexp.owner_id run data modify storage vexp:temp bow_probe_owner set from entity @s data.vexp.owner_id
execute if data entity @s data.vexp.owner_id as @e[type=minecraft:marker,tag=vexp.bow_probe,distance=..6] if score @s vexp.id = #temp vexp.id run data modify entity @s data.vexp.owner_id set from storage vexp:temp bow_probe_owner
