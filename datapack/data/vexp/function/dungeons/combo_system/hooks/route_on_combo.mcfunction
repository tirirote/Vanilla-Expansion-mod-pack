...
$execute if data storage vexp:dungeons.weapon combo_params{quality:"amethyst"} as @e[tag=vexp.hitted,distance=..8] at @s run function vexp:dungeons/$(item)/amethyst/on_combo

# 2. Default route (Normal quality or undefined)
$execute unless data storage vexp:dungeons.weapon combo_params{quality:"netherite"} unless data storage vexp:dungeons.weapon combo_params{quality:"echo"} unless data storage vexp:dungeons.weapon combo_params{quality:"amethyst"} as @e[tag=vexp.hitted,distance=..8] at @s run function vexp:dungeons/$(item)/on_combo
