...
# Feedback
execute positioned ^ ^ ^3.5 if entity @e[predicate=vexp:is_target,distance=..3.5] unless score @s vexp.id = #combo_owner_id vexp.id at @s run function vexp:dungeons/fx/hits/netherite_hit

# Dash damage and knockback
execute positioned ^ ^ ^3.5 as @e[predicate=vexp:is_target,distance=..3.5] unless score @s vexp.id = #combo_owner_id vexp.id at @s run function vexp:dungeons/weapons/spear/dash_damage {damage: 5}
