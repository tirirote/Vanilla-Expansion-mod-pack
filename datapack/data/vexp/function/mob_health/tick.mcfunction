...
execute as @e[tag=vexp.health_init] at @s run function vexp:mob_health/update

# Fixed 0 HP bug
execute as @e[tag=vexp.health_init,tag=!vexp.custom_mob] if score @s vexp.hp matches ..0 run kill @s
...
execute as @e[tag=!vexp.health_init,predicate=vexp:is_target,type=!minecraft:player,tag=!vexp.custom_block.dummy_mob] at @s if entity @p[distance=..12] if data entity @s Health run function vexp:mob_health/update
