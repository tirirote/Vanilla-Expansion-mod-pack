...
$execute at @s if entity @e[predicate=vexp:is_target,distance=..$(homing_distance),tag=!vexp.$(exclude_tag),limit=1,sort=nearest] run tp @s ~ ~ ~ facing entity @e[predicate=vexp:is_target,distance=..$(homing_distance),tag=!vexp.$(exclude_tag),limit=1,sort=nearest] feet
