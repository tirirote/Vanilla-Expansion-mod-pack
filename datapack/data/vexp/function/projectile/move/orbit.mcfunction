...
# Llamado con 'with entity @s data.proj' → $(radius), $(height) disponibles.
...
$execute if score #orbit_face vexp.math matches 1.. as @a if score @s vexp.id = #orbit_owner vexp.math at @s positioned ^ ^ ^$(radius) run tp @e[tag=vexp.orbit_temp,limit=1] ~ ~$(height) ~

# face_player=0: radius follows projectile own rotation (camera-independent tornado behavior).
$execute unless score #orbit_face vexp.math matches 1.. as @a if score @s vexp.id = #orbit_owner vexp.math at @s rotated as @e[tag=vexp.orbit_temp,limit=1] positioned ^ ^ ^$(radius) run tp @e[tag=vexp.orbit_temp,limit=1] ~ ~$(height) ~
