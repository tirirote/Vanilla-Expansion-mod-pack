...
data modify storage vexp:dungeons.weapon params set value {item_id:"netherite_axe", name:"Hacha de Netherite", model:"vexp:none", quality:"netherite", dmg:12.0, spd:-3.2, combo_dmg:14, combo_cooldown:26, combo_range:3.5, combo_reach:1.8, reach_mod:-1.0}
execute as @p[tag=vexp.projectile_owner,limit=1] run function vexp:dungeons/axe/give_macro with storage vexp:dungeons.weapon params
