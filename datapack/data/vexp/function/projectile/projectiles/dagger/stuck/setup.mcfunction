...
execute if entity @s[tag=vexp.amethyst] run summon item_display ~ ~ ~ {Tags:["vexp.dagger_stuck_visual","vexp.temp_stuck_visual"],item:{id:"minecraft:netherite_sword",components:{"minecraft:custom_model_data":{strings:["vexp:amethyst_dagger"]}}},transformation:{translation:[0f,0f,0f],scale:[0.8f,0.8f,0.8f],left_rotation:[-.25f,.5f,.5f,0f],right_rotation:[0f,0f,0f,1f]},interpolation_duration:1,teleport_duration:1}
