...
execute if entity @s[tag=vexp.dagger_projectile] positioned ~ ~-1 ~ if entity @e[type=!player,type=!item,type=!marker,type=!interaction,type=!item_display,type=!area_effect_cloud,distance=..1.5,limit=1,sort=nearest] run function vexp:projectile/on_hit_mob

# Otros proyectiles usan detección estándar
execute if entity @s[tag=!vexp.dagger_projectile] if entity @e[type=!player,type=!item,type=!marker,type=!interaction,type=!item_display,type=!area_effect_cloud,distance=..1.5,limit=1,sort=nearest] run function vexp:projectile/on_hit_mob
