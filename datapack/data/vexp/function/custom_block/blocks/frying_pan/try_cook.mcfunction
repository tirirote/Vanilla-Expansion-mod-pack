...
execute if score #did_cook vexp.math matches 1 run particle large_smoke ^ ^.2 ^ 0.1 0.1 0.1 0.01 1

# 4. Añadir animación de jitter
execute if score #did_cook vexp.math matches 1 run tag @s add vexp.block.jitter
execute if score #did_cook vexp.math matches 1 run scoreboard players set @s vexp.block_anim 10
