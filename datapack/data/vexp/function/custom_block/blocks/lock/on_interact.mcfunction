...
execute if data entity @s data.vexp{key_name:""} if items entity @a[tag=vexp.lock_user,limit=1] weapon.mainhand minecraft:poisonous_potato[minecraft:custom_model_data={strings:["vexp:door_key"]}] if data entity @a[tag=vexp.lock_user,limit=1] SelectedItem.components."minecraft:custom_name" run data modify entity @s data.vexp.key_name set from entity @a[tag=vexp.lock_user,limit=1] SelectedItem.components."minecraft:custom_name"

execute if data entity @s data.vexp{key_name:""} if items entity @a[tag=vexp.lock_user,limit=1] weapon.mainhand minecraft:poisonous_potato[minecraft:custom_model_data={strings:["vexp:door_key"]}] unless data entity @a[tag=vexp.lock_user,limit=1] SelectedItem.components."minecraft:custom_name" run data modify entity @s data.vexp.key_name set from entity @a[tag=vexp.lock_user,limit=1] SelectedItem.components."minecraft:item_name".text
