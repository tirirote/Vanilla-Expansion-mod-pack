...
execute if data entity @s data.vexp{has_lock:0} if items entity @a[tag=vexp.door_user,limit=1] weapon.mainhand item_frame[minecraft:entity_data~{id:"item_frame",data:{vexp:{type:"lock"}}}] run function vexp:custom_block/blocks/door/install_lock
