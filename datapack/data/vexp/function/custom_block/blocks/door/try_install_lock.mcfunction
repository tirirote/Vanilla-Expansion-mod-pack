...
execute if data entity @s data.vexp{has_lock:0} if data entity @a[tag=vexp.door_user,limit=1] SelectedItem{id:"minecraft:item_frame",components:{"minecraft:custom_model_data":{"strings": ["vexp:lock"]}}} run function vexp:custom_block/blocks/door/install_lock
